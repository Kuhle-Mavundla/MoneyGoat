 ...
 * Provides functionality to retrieve and save monthly spending/saving targets.
 */
class GoalViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * Retrieves the budget goal for a specific user, month, and year.
     * @return LiveData containing the BudgetGoal if it exists.
     */
    ...
    /**
     * Saves or updates a budget goal for a specific month and year.
     * Uses an upsert logic by checking for existing entries first.
     */
    fun saveGoal(userId: Long, month: Int, year: Int, minGoal: Double, maxGoal: Double) {
        viewModelScope.launch {
            try {
                ...
                // If existing record found, use its ID to perform an update; otherwise insert new.
                dao.insertOrUpdate(BudgetGoal(
                    id = existing?.id ?: 0, 
                    userId = userId, 
                    month = month, 
                    year = year, 
